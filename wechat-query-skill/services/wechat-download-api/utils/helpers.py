...
def is_video_page(html: str) -> bool:
    """
    检测是否为视频页。
    这类页面通常带有 js_video_page_title，正文容器 js_content 只是 share_notice 占位。
    """
    return (
        'id="js_video_page_title"' in html or
        'class="rich_media_title js_video_channel_title"' in html or
        'id="video_share_global_info"' in html
    )


def is_placeholder_content(content: str) -> bool:
    """
    判断提取出的 HTML 是否只是占位提示而非正文。
    常见于视频页，js_content 里只有 share_notice_wrp。
    """
    if not content:
        return True
    if 'share_notice_wrp' not in content and 'id="js_common_share_desc_wrap"' not in content:
        return False
    text = html_to_text(re.sub(r'<script[^>]*>[\s\S]*?</script>', '', content, flags=re.IGNORECASE))
    return len(text.strip()) < 20


def _parse_wechat_datetime(value: str) -> int:
    """将微信页面中的 YYYY-MM-DD HH:MM 时间字符串转为 Unix 时间戳。"""
    try:
...
def _extract_video_page_content(html: str) -> Dict:
    """
    提取视频页的可用信息。
    这类页面往往没有可直接抽取的正文，返回封面 + 说明性文本，避免误报 share_notice 为正文。
    """
    import html as html_module
    ...
    title_match = re.search(r"title:\s*JsDecode\('([^']*)'\)", html)
    if title_match:
        title = _jsdecode_unescape(title_match.group(1)).strip()

    author_match = re.search(r"nick_name:\s*JsDecode\('([^']*)'\)", html)
    if author_match:
        author = _jsdecode_unescape(author_match.group(1)).strip()

    desc_match = re.search(r"desc:\s*JsDecode\('([^']*)'\)", html)
    if desc_match:
        desc = _jsdecode_unescape(desc_match.group(1)).strip()

    cover_match = re.search(r"cdn_url:\s*JsDecode\('([^']*)'\)", html)
    if cover_match:
        cover = _jsdecode_unescape(cover_match.group(1)).strip()

    create_match = re.search(r"create_time:\s*JsDecode\('([^']*)'\)", html)
    if create_match:
        create_time = _jsdecode_unescape(create_match.group(1)).strip()

    html_parts = []
    if cover:
        safe_cover = html_module.escape(cover, quote=True)
        html_parts.append(
            f'<p style="text-align:center;margin:0 0 12px">'
            f'<img src="{safe_cover}" data-src="{safe_cover}" '
            f'style="max-width:100%;height:auto;border-radius:8px" />'
            f'</p>'
        )

    note = desc or "该页面为视频文章，微信页面未提供可直接提取的正文文本。"
    html_parts.append(
        f'<p style="margin:0 0 8px;line-height:1.8">{html_module.escape(note)}</p>'
    )
    ...
    if create_time:
        meta_lines.append(f"发布时间：{create_time}")
    if meta_lines:
        html_parts.append(
            f'<p style="margin:0;color:#888;font-size:13px">{html_module.escape(" | ".join(meta_lines))}</p>'
        )

    plain_parts = [note]
    if author:
        plain_parts.append(f"作者：{author}")
    if create_time:
        plain_parts.append(f"发布时间：{create_time}")

    return {
        'title': title,
        'author': author,
        'publish_time': _parse_wechat_datetime(create_time) if create_time else 0,
...
def extract_article_info(html: str, params: Optional[Dict] = None) -> Dict:
    ...
    if is_image_text_message(html):
    ...
    elif is_video_page(html):
        video_data = _extract_video_page_content(html)
        content = video_data['content']
        images = video_data['images']
        plain_content = video_data['plain_content']
        if not title and video_data.get('title'):
            title = video_data['title']
        if not author and video_data.get('author'):
            author = video_data['author']
        if not publish_time and video_data.get('publish_time'):
        ...
        if content and is_placeholder_content(content):
            content = ''
            images = []
...
def extract_account_name(html: str) -> str:
    ...
    与作者不同，公众号名称优先从公众号昵称相关字段中提取，
    不优先使用 og:article:author，避免拿到文章作者署名。
    """
    candidates = [
        re.search(r'<a[^>]*id="js_name"[^>]*>([\s\S]*?)</a>', html, re.IGNORECASE),
        re.search(r'var\s+nickname\s*=\s*"([^"]+)"', html),
        re.search(r'<a[^>]*class=[^>]*rich_media_meta_nickname[^>]*>([^<]+)</a>', html, re.IGNORECASE),
        re.search(r'nickname\s*[:=]\s*"([^"]+)"', html),
    ]
...
def has_article_content(html: str) -> bool:
    ...
    if any(marker in html for marker in element_markers):
        return True
    if is_image_text_message(html) or is_short_content_message(html) or is_audio_message(html) or is_video_page(html):
        return True
    return False
