...
from utils.helpers import extract_account_name, extract_article_info, parse_article_url, is_image_text_message, has_article_content, get_client_ip
...
class ArticleData(BaseModel):
    ...
    source_url: Optional[str] = Field(None, description="原始文章链接")
...
def _parse_article_with_fallback_params(url: str, html: str) -> dict:
    ...
    **返回字段：**
    ...
    try:
        ...
        if not has_article_content(html):
            ...
            if "请登录" in html:
                auth_manager.mark_invalid("article", "wechat page requires login")
        ...
        params = _parse_article_with_fallback_params(article_request.url, html)

        article_data = extract_article_info(html, params)
        article_data["account_name"] = extract_account_name(html)
        article_data["source_url"] = article_request.url
...
@router.post("/article/fetch", response_model=ArticleResponse, summary="带登录态抓取并解析单篇文章")
async def fetch_article(article_request: ArticleRequest, request: Request):
    """
    使用当前登录凭证抓取并解析单篇文章。

    相比 `/api/article`，该接口会显式带上当前 token/cookie，
    更适合作为 agent 处理“用户直接给出文章链接”的主链路。
    """
    client_ip = get_client_ip(request)
    allowed, error_msg = rate_limiter.check_rate_limit(client_ip, "/api/article")
    if not allowed:
    ...
    try:
        logger.info("[ArticleFetch] request from %s: %s", client_ip, article_request.url[:80])

        html = await fetch_article_content(
            article_request.url,
            timeout=60,
            wechat_token=credentials.get("token", ""),
            wechat_cookie=credentials.get("cookie", ""),
        )

        if not html:
            return {
                "success": False,
                "error": "无法获取文章内容。可能原因：链接无效、内容为空或抓取失败。"
            }
        ...
        if not has_article_content(html):
            ...
            if "请登录" in html:
                auth_manager.mark_invalid("article_fetch", "wechat page requires login")
                await webhook.notify('login_expired', {
                    'account': credentials.get('nickname', ''),
                    'url': article_request.url
                })
