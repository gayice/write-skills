    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if result.get("base_resp", {}).get("ret") == 0:
                ...
                if (
                    "login" in err_msg_lower or
                    "invalid session" in err_msg_lower or
                    "session expired" in err_msg_lower or
                    "session invalid" in err_msg_lower or
                    ret_code == 200003
                ):
                    auth_manager.mark_invalid("searchbiz", f"ret={ret_code}, msg={err_msg}")
                return SearchResponse(
                    success=False,
                    error=f"搜索失败: {err_msg}"
                )
