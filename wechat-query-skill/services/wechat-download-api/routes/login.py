...
from utils.relogin_manager import relogin_manager
...
@router.post("/relogin/start", summary="发起服务端托管重登")
async def start_relogin():
    """
    创建一条新的服务端重登会话。
    skill 可直接使用返回的二维码文件路径，通过 MEDIA 发送给用户。
    """
    try:
        data = await relogin_manager.start()
        return {"success": True, "data": data}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"发起重登失败: {str(e)}")


@router.get("/relogin/{request_id}/qrcode", summary="获取重登二维码")
async def get_relogin_qrcode(request_id: str):
    """返回重登会话二维码图片。"""
    try:
        qrcode_path = relogin_manager.get_qrcode_path(request_id)
    except KeyError:
        raise HTTPException(status_code=404, detail="未找到重登会话")
    ...
    return FileResponse(
        ...
        media_type=relogin_manager.get_qrcode_media_type(request_id),
        filename=qrcode_path.name,
    )


@router.get("/relogin/{request_id}/status", summary="查询重登会话状态")
async def get_relogin_status(request_id: str):
    """轮询重登会话状态，必要时自动完成登录并覆盖旧凭证。"""
    try:
        data = await relogin_manager.get_status(request_id)
        return {"success": True, "data": data}
    except KeyError:
        raise HTTPException(status_code=404, detail="未找到重登会话")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"查询重登状态失败: {str(e)}")


@router.post("/relogin/{request_id}/cancel", summary="取消重登会话")
async def cancel_relogin(request_id: str):
    """取消未完成的重登会话。"""
    try:
        data = await relogin_manager.cancel(request_id)
        return {"success": True, "data": data}
    except KeyError:
        raise HTTPException(status_code=404, detail="未找到重登会话")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"取消重登失败: {str(e)}")
